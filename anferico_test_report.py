...
model = BertForMaskedLM.from_pretrained(model_name, config=config).to("cuda")

qdrant_url = "http://192.168.10.142:6333/"
collection_name = "testcase_anf"
client = QdrantClient(qdrant_url,timeout=None)
...
# Iterate over projects from 1 to 15
for project_id in range(1, 16):
    project_key = f"project_{project_id}"
    if project_key not in search_data:
        continue  # Skip if project key doesn't exist

    project_data_list = search_data[project_key]
    ...
    for project_data in project_data_list:
        ...
        for criterion_idx, search_criterion in enumerate(project_data["search_criteria"], start=1):
            ...
            inputs_user = tokenizer(search_criterion, return_tensors="pt", padding=False, truncation=True,
                                    max_length=512).to("cuda")

            # Get the model output for the search criterion
            with torch.no_grad():
                outputs_user = model(**inputs_user)

            # Get the pooled output (contextualized representation of the search criterion)
            last_hidden_state = outputs_user['hidden_states'][-1]
            ...
            for limit in limit_list:
                ...
                for j, result in enumerate(search_results):
                    for patent_number_ddb in project_data["test_case"].get("patent_number_ddb_ddb", []):
                        if first_position is None and (
                                patent_number_ddb in result.payload.get("patent_number_ddb_ddb", []) or patent_number_ddb in
                                result.payload.get("patent_number_original_national_office", [])):
                            first_position = j + 1
                        if patent_number_ddb in result.payload.get("patent_number_ddb_ddb",
                                                                    []) or patent_number_ddb in result.payload.get(
                            "patent_number_original_national_office", []):
                            position = j + 1  # Get the position of the current matching patent number
                            highest_rank = min(highest_rank,
                                                position)  # Update highest_rank with the lowest position found so far
                            latest_occurrence_rank = max(latest_occurrence_rank,
                                                         position)  # Update latest_occurrence_rank with the highest position found so far
                ...
                for i, patent_number_ddb in enumerate(project_data["test_case"].get("patent_number_ddb_ddb", []),
                                                      start=1):
                    found = False
                    for j, result in enumerate(search_results):
                        if patent_number_ddb in result.payload.get("patent_number_ddb_ddb", []) or patent_number_ddb in \
                                result.payload.get("patent_number_original_national_office", []):
                            position = j + 1
                            criterion_data["Patent_no"][
                                patent_numbers.index(patent_number_ddb)] = f"{patent_number_ddb} - {position}"
                            print(
                                f"Patent number {patent_number_ddb} for search criterion {criterion_idx} was found at position {position} in top {limit} results.")
                            found = True
                            break
                    if not found:
                        criterion_data["Patent_no"][
                            patent_numbers.index(patent_number_ddb)] = f"{patent_number_ddb} - NA"
                    if found:
                        retrieved_documents_dict[limit].add(patent_number_ddb)
                # Similarly, iterate over the other patent number field
                for i, patent_number_original in enumerate(
                        project_data["test_case"].get("patent_number_original_national_office", []), start=1):
                    found = False
                    # Check if the patent number exists in the search results
                    for j, result in enumerate(search_results):
                        if patent_number_original in result.payload.get("patent_number_ddb_ddb", []) or \
                                patent_number_original in result.payload.get("patent_number_original_national_office",
                                                                              []):
                            position = j + 1
                            criterion_data["Patent_no"][
                                patent_numbers.index(patent_number_ddb)] = f"{patent_number_ddb} - {position}"
                ...
                print(f"Mean Reciprocal Rank lower:", round(MRR_lower, 8))
                # Update MRR, Precision, and Recall fields in criterion data
                criterion_data[f"MRRFirstdoc"] = f"{MRR_upper:.8f}" if MRR_upper > 0 else "NA"
...
if os.path.exists(output_json_file_path) and os.path.getsize(output_json_file_path) > 0:
    with open(output_json_file_path, "r") as existing_output_file:
        try:
            existing_output_data = json.load(existing_output_file)
        except json.JSONDecodeError:
            print("Existing output file does not contain valid JSON data.")
